...
export class FXOverAction implements IAction {
    ...
    public init(): Promise<void> {
        this.cron = cron.schedule('0 0 */2 * * *', () => {
            ...
            .end((error, result) => {
                if (error)
                    return this.error.createError('99', 'FXOverAPI', {}, {error, result});
                ...
                .then((snapshots) => {
                    ...
                    snapshots.forEach((snapshot) => {
                        ...
                        if (area.action.name === 'FXOver') {
                            const data = area.action.data as IFXOver;
    ...
    public getDescription(): string {
        return "Action that triggers when a currency is over a certain value";
    }
...
inject.register("FXOverAction", FXOverAction);